...
# Adjusting the timestamp scale and plot colors
plot_data['timestamp'] = plot_data['timestamp'] * 10  # Convert from /100 to /1000 scale
...
plt.plot(plot_data['timestamp'], plot_data['ratio1'], label='Data 1 (compute1num / recvNum)', marker='o', color='blue')
...
plt.plot(plot_data['timestamp'], plot_data['ratio2'], label='Data 2 (compute1num / recvNum)', marker='x', color='red')
...
plt.plot(plot_data['timestamp'], plot_data['ratio3'], label='Data 2 ((compute1num + compute2num) / recvNum)', marker='s', color='green')
...
plt.ylabel('Ratios')
plt.title('Comparison of Compute Ratios Over Time with Median Lines and Annotations')
plt.legend()
plt.grid(True)
plt.show()
