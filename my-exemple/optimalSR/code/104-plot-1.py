...
plt.plot(filtered_data['timestamp_seconds'], filtered_data['ratio1'], label='Dataset 1: compute1num/recvNum', color='tab:red')
plt.axhline(y=median_ratio1, color='tab:red', linestyle='--', label=f'Median (Dataset 1: compute1num/recvNum): {median_ratio1:.2f}')
plt.plot(filtered_data['timestamp_seconds'], filtered_data['ratio2'], label='Dataset 1: (compute1num + compute2num)/recvNum', color='tab:blue')
plt.axhline(y=median_ratio2, color='tab:blue', linestyle='--', label=f'Median (Dataset 1: (compute1num + compute2num)/recvNum): {median_ratio2:.2f}')
plt.plot(new_filtered_data['timestamp_seconds'], new_filtered_data['ratio1_new'], label='Dataset 2: compute1num/recvNum', color='tab:green')
plt.axhline(y=median_ratio1_new, color='tab:green', linestyle='--', label=f'Median (Dataset 2: compute1num/recvNum): {median_ratio1_new:.2f}')
