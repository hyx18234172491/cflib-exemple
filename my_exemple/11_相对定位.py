...
sequences_1 = [
    (0, 0, high, 0),
    (1, 0, high, 0),
    (solve_x(0.25), -0.25, high, 0),  # 旋转
    (solve_x(0.50), -0.50, high, 0),  # 旋转
    (solve_x(0.75), -0.75, high, 0),  # 旋转
    (solve_x(1), -1, high, 0),  # 旋转
    (2, 0, high, 0),  # 转向
]
...
def run_sequence(scf, sequence):
    try:
        ...
        for position in sequence:
            ...
            # x ,y ,z, 不知道, time
            commander.go_to(position[0], position[1], position[2], position[3], 2)
            time.sleep(3)
        land(cf, sequence[-1])
