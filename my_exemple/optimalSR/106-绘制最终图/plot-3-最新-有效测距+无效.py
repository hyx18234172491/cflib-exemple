...
# 绘制每个系列的数据
bars1 = ax1.bar(x - 1.5 * (bar_width + spacing), SR1_receive_rate, width=bar_width, label='SR1 Receive',
                edgecolor='black')
bars3 = ax1.bar(x - 0.5 * (bar_width + spacing), SR1_ranging_rate, width=bar_width, label='SR1 Ranging',
                edgecolor='black')
bars2 = ax1.bar(x + 0.5 * (bar_width + spacing), SR2_receive_rate, width=bar_width, label='SR2 Ranging all',
                edgecolor='black')

bars4 = ax1.bar(x + 1.5 * (bar_width + spacing), SR2_ranging_rate, width=bar_width, color="#FEDC5E",
                label='SR2 Ranging valid', edgecolor='black')
...
line3, = ax2.plot(x, SR1_ranging_count, color='tab:orange', marker='^', markersize=6, label='SR1 Ranging')
line2, = ax2.plot(x, SR2_receive_count, color='tab:green', marker='s', markersize=6, label='SR2 Ranging all')
line4, = ax2.plot(x, SR2_ranging_count, color='#FEDC5E', marker='D', markersize=6, label='SR2 Ranging valid')
# 添加折线图的图例
lines = [line1, line3, line2, line4]
labels_lines = [line.get_label() for line in lines]
ax2.legend(lines, labels_lines, loc='upper center', bbox_to_anchor=(0.5, 1.18), ncol=4, fontsize='13')
...
ax1.legend(bars, labels_bars, loc='upper center', bbox_to_anchor=(0.5, 1.1), ncol=4, fontsize='13')

# 显示图形
plt.subplots_adjust(left=0.08, right=0.9, top=0.88, bottom=0.11)
plt.show()
