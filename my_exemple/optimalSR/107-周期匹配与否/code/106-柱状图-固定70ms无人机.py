...
def process_data(df):
    ...
    df_cleaned['recvNum2_ratio'] = df_cleaned['Statistic.recvNum0'] / df_cleaned['Statistic.recvSeq0']
    df_cleaned['computeNum2_ratio'] = (df_cleaned['Statistic.compute1num0']) / \
                                      df_cleaned['Statistic.recvSeq0']
    df_cleaned['computeNum2_and1_ratio'] = (df_cleaned['Statistic.compute1num0'] + df_cleaned[
        'Statistic.compute2num0']) / \
                                           df_cleaned['Statistic.recvSeq0']
    last_row = df_cleaned.iloc[-1]
    recvNum2_ratio_median = round(last_row['recvNum2_ratio'], 2)
    computeNum2_ratio_median = round(last_row['computeNum2_ratio'], 2)
    computeNum2_and1_ratio_median = round(last_row['computeNum2_and1_ratio'], 2)
